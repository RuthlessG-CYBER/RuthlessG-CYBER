export default function SidebarRight() {
  ...
  return (
    <aside className="w-80 border-l border-white/10 bg-white/5/50 flex flex-col h-[calc(100vh-53px)] overflow-y-auto">
      <div className="p-4">
        ...
        <p className="text-xs text-gray-400 mb-4">Using Claude Code</p>

        <div className="space-y-3">
          {jobs.map((job, idx) => (
            <div key={idx} className="bg-[#0f0f11] border border-white/10 rounded-lg p-3 hover:border-white/20 transition-colors shadow-sm cursor-pointer">
               <div className="flex gap-3">
                  <div className="w-8 h-8 rounded bg-white/10 flex items-center justify-center shrink-0 font-bold text-gray-400 text-sm">
                    {job.company.charAt(0)}
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-white leading-tight">{job.title}</h4>
